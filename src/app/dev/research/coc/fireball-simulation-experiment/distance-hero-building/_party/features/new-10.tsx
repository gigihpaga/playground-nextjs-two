...
// v10 - copy dari v6 Line-distance diperbarui secara real-time
...
type BallShape = { id: number; x: number; y: number; type: string };

export function New10() {
    const [balls, setBalls] = useState<BallShape[]>([
    ...
    const ballDraggingTemp = useRef<BallShape | null>(null);
        ...
        <div className="relative h-screen w-screen overflow-hidden bg-gray-900">
            ...
            >
                <DndContext
                    onDragStart={(event) => {
                        setIsDragging(true);
                        const ballActive = balls.find((ball) => ball.id === event.active.id);
                        if (!ballActive) return;
                        ballDraggingTemp.current = ballActive;
                    }}
                    onDragMove={(event) => {
                        const { active, delta } = event;

                        const ballTemp = ballDraggingTemp.current;
                        ...
                        setBalls((prevBalls) => {
                            return prevBalls.map((ball) => {
                                if (ball.id === active.id) {
                                    ...
                                    let newX = ballTemp.x + delta.x;
                                    let newY = ballTemp.y + delta.y;
