...
// v5 - Bola hanya bisa drag-and-drop di dalam Board
...
const BoardSize = 500;
const GridSize = 25;
const BallSize = 40; // Ukuran bola

export function New5() {
    const [balls, setBalls] = useState([{ id: 1, x: 100, y: 100 }]);
    const [scale, setScale] = useState(1);
    const [position, setPosition] = useState({ x: 0, y: 0 });
    const [isDragging, setIsDragging] = useState(false);
    const viewportRef = useRef<HTMLDivElement>(null);
    const boardRef = useRef<HTMLDivElement>(null); // Ref untuk Board
                        ...
                        setBalls((prevBalls) => {
                            return prevBalls.map((ball) => {
                                if (ball.id === active.id) {
                                    ...
                                    // Batasi agar bola tidak keluar dari Board
...
function Ball({ id, x, y }: { id: number; x: number; y: number }) {
    ...
    return (
        <motion.div
            ...
            {...listeners}
            {...attributes}
            className={`absolute w-[${GridSize * 2}px] h-[${GridSize * 2}px] bg-red-500 rounded-full`}
            style={{
