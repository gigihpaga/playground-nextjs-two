...
// v9 - Perbaikan posisi bola agar mengikuti lokasi pointer dengan benar
            ...
            >
                <DndContext
                    ...
                    onDragMove={(event) => {
                        ...
                        console.log('onDragMove', delta.x, delta.y);
                        return;
                        ...
                        setBalls((prevBalls) =>
                            prevBalls.map((ball) =>
                                ball.id === active.id
                                    ? {
                                          ...ball,
                                          x: Math.max(0, Math.min(ball.x + delta.x, BoardSize - BallSize)),
                                          y: Math.max(0, Math.min(ball.y + delta.y, BoardSize - BallSize)),
                                      }
                                    : ball
                            )
                        );
                        ...
                        setBalls((prevBalls) => {
                            return prevBalls.map((ball) => {
                                if (ball.id === active.id) {
                                    ...
                                    console.log('onDragEnd', { ...ball, x: newX, y: newY });
...
function _Ball({ id, x, y, type }: { id: number; x: number; y: number; type: string }) {
    const { attributes, listeners, setNodeRef, transform } = useDraggable({ id });
