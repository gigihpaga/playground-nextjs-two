...
export const clampPosition = (position: XYPosition = { x: 0, y: 0 }, extent: CoordinateExtent, dimensions: Partial<Dimensions>) => ({
...
export const calcAutoPan = (pos: XYPosition, bounds: Dimensions, speed: number = 15, distance: number = 40): number[] => {
...
export const getBoundsOfRects = (rect1: Rect, rect2: Rect): Rect => boxToRect(getBoundsOfBoxes(rectToBox(rect1), rectToBox(rect2)));
...
export const isRectObject = (obj: any): obj is Rect => isNumeric(obj.width) && isNumeric(obj.height) && isNumeric(obj.x) && isNumeric(obj.y);
 ...
 *1200, 800, 0.5, 2);
 */
export const getViewportForBounds = (bounds: Rect, width: number, height: number, minZoom: number, maxZoom: number, padding: number): Viewport => {
