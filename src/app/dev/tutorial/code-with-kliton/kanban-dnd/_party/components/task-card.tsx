    ...
    if (isDragging) {
        return (
            <div
                ref={setNodeRef}
                style={style}
                className="bg-[#0D1117] [&>:is(textarea,p)::-webkit-scrollbar-thumb]:bg-sky-500 overflow-hidden group/TaskCard p-2.5 h-[100px] min-h-[100px] items-center flex text-left rounded-xl ring-2 ring-inset ring-green-500 cursor-grab relative opacity-40"
            >
    ...
    if (!isEditMode) {
        return (
            ...
            <div
                ...
                className="bg-[#0D1117] [&>:is(textarea,p)::-webkit-scrollbar-thumb]:bg-sky-500 overflow-hidden group/TaskCard p-2.5 h-[100px] min-h-[100px] items-center flex text-left rounded-xl hover:ring-2 hover:ring-inset hover:ring-rose-500 cursor-grab relative"
            >
                <p className="my-auto h-[90%] w-full overflow-y-auto overflow-x-hidden whitespace-pre-wrap">{task.content}</p>
                <button
                    onClick={() => {
                ...
                >
                    <TrashIcon className="size-4 stroke-white group-hover:stroke-rose-500" />
                </button>
            </div>
        );
    }
