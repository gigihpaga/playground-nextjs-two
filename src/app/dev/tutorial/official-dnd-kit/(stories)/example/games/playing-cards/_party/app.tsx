...
import { Sortable } from '@/app/dev/tutorial/official-dnd-kit/(stories)/presets/Sortable/Sortable';
// import { MultipleContainers } from '../../../2 - Presets/Sortable/MultipleContainers';
import { MultipleContainers } from '@/app/dev/tutorial/official-dnd-kit/(stories)/presets/Sortable/MultipleContainers';
// import { PlayingCard, getDeckOfCards } from './PlayingCard';
import { PlayingCard, getDeckOfCards } from './components/playing-card';

// export default { title: 'Examples/2D Games/Playing Cards' };
    ...
    return (
        ...
        `}
            ...
            <MultipleContainers
                ...
                items={decks}
                renderItem={({ value, isDragOverlay, isDragging, isSorting, isFadeIn, index, listeners, ref, style, transform, transition }) => {
                    // console.log(style);
                    return (
                        <PlayingCard
                            value={(value as string).substring(2, (value as string).length)}
                            ...
                            transform={transform}
                            transition={transition}
                            isMountAnimation={!isDragOverlay && !isMounted}
                            {...listeners}
                        />
                    );
                }}
                ...
                getItemStyles={({ index, overIndex, isDragging, containerId, isDragOverlay }) => {
                    ...
                    const calculatedZIndex = isDragOverlay ? undefined : isDragging ? deck.length - overIndex : deck.length - index;
                    console.log('isDragOverlay', isDragOverlay, `zIndex:${calculatedZIndex} `);
                    return {
