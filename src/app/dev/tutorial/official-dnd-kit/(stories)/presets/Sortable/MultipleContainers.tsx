...
import { Item, Container, ContainerProps } from '@/app/dev/tutorial/official-dnd-kit/_party/components';
...
function DroppableContainer({ children, columns = 1, disabled, id, items, style, ...props }: DroppableContainerProps) {
...
interface MultipleContainersProps extends React.HtmlHTMLAttributes<HTMLDivElement> {
    /** default: false */
    isAdjustScale?: boolean; // dibutukan oleh MultipleContainers
    cancelDrop?: CancelDrop; // dibutukan oleh MultipleContainers
    /** default: 3 */
    itemCount?: number; // dibutukan oleh MultipleContainers
    ...
    isMinimal?: boolean; // dibutuhkan oleh DroppableContainer & MultipleContainers
    columns?: number; // dibutuhkan oleh DroppableContainer
    containerStyle?: React.CSSProperties; // dibutuhkan oleh DroppableContainer
    isScrollable?: boolean; // dibutuhkan oleh DroppableContainer
    /** default: false */
    handle?: boolean; // dibutuhkan oleh SortableItem
    getItemStyles?(args: {
    ...
    wrapperStyle?(args: { index: number }): React.CSSProperties; // dibutuhkan oleh SortableItem
    renderItem?: ItemProps['renderItem']; // dibutukan oleh MultipleContainers & SortableItem
    /** default: verticalListSortingStrategy */
    strategy?: SortingStrategy; // dibuthkan oleh EachItem
    /** default false */
    items?: Items; // dibutuhkan oleh MultipleContainers & EachColumn
    isVertical?: boolean; // dibuthkan oleh EachColumn
}
...
export function MultipleContainers(_props: MultipleContainersProps) {
    const {
        isAdjustScale = false,
        itemCount = 3,
        cancelDrop,
    ...
    return (
        ...
        >
            <div
                aria-description="MultipleContainers"
                style={{
                    display: 'inline-grid',
                ...
                >
                    {containers.map((containerId) => (
                        <DroppableContainer
                            key={containerId}
                            id={containerId}
                            label={isMinimal ? undefined : `Column ${containerId}`}
                            columns={columns}
            ...
            {isTrashable && activeId && !containers.includes(activeId) ? <Trash id={TRASH_ID} /> : null}
        </DndContext>
    );
...
function SortableItem({ disabled, id, index, handle, renderItem, getItemStyles, containerId, getIndex, wrapperStyle }: SortableItemProps) {
